...
export function HistoryCard({
  ...
  const hasStats =
    item.video_view_count !== undefined ||
    item.video_comment_count !== undefined ||
    item.video_like_count !== undefined;
          ...
          <div className="mt-2 flex items-center gap-3 text-xs text-gray-500">
            {item.video_view_count !== undefined && (
              <span className="flex items-center gap-1">
                ...
                {formatCount(item.video_view_count)}
              </span>
            )}
            {item.video_comment_count !== undefined && (
              <span className="flex items-center gap-1">
                ...
                {formatCount(item.video_comment_count)}
              </span>
            )}
            {item.video_like_count !== undefined && (
              <span className="flex items-center gap-1">
                ...
                {formatCount(item.video_like_count)}
              </span>
            )}
          </div>
        )}
        ...
        {hasSentiment && (
          <div className="mt-3">
            ...
            <div className="mt-1.5 flex flex-wrap gap-x-3 text-xs">
              <span className="text-emerald-600">● Positive {item.sentiment_positive}%</span>
              <span className="text-gray-400">● Neutral {item.sentiment_neutral}%</span>
              <span className="text-rose-500">● Negative {item.sentiment_negative}%</span>
            </div>
          </div>
        )}
